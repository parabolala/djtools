...
class TestParseXML(unittest.TestCase):
    def test_parse_cue_point(self):
        ...
        actual = [models.CuePoint.parse(cp)
                  for cp in root.findall('POSITION_MARK')]
    ...
    def test_parse_xml(self):
        expected = [
            models.Track(
                ...
                CuePoints=[
                    models.CuePoint(
                        Num=0, Start=1.444, Red=230, Blue=40, Green=40),
                    models.CuePoint(
                        Num=1, Start=12.098, Red=40, Blue=20, Green=226),
                    models.CuePoint(
                        Num=2, Start=172.113, Red=230, Blue=40, Green=40),
                    models.CuePoint(
                        Num=3, Start=182.814, Red=40, Blue=20, Green=226),
                ],
            ),
        ]
